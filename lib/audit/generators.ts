...
export function generateFixesMarkdown(result: AuditResult): string {
  ...
  lines.push(
    ...
    ...(result.robots.sitemaps.length > 0
      ? [
          '',
          `> **You already have a sitemap** (\`${result.robots.sitemaps[0]}\`). The one in this kit is not`,
          '> a duplicate: it is the cleaned, deduplicated canonical set — trailing-slash and',
          '> tracking-parameter variants collapsed, redirects and error pages dropped, with',
          '> tiered `<priority>` weights and a current `<lastmod>`. Use it as an optimised',
          '> canonical alternative. On a site larger than this audit crawled, merge its',
          '> structure into your existing file rather than replacing it outright.',
          '',
        ]
      : []),
 ...
 * files baked into the record, so an audit run before a new generator was
 * added carries an object with the OLD set of keys — and the download route
 * trusted it. Reading a key that is not there yielded `undefined`, which
 * `Buffer.from` then rejected, so the whole archive failed rather than just
 * the one file.
 *
 * Checking the shape instead of merely checking for null means an old record
 * silently regenerates and the customer gets all five files.
 */
export function isCompleteFixKit(
